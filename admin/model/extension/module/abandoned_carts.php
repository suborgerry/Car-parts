...
class ModelExtensionModuleAbandonedCarts extends Model {
	...
	public function getOrders($data = array()) {
		...
		if ($this->config->get('module_abandoned_carts_criteria')){
			foreach ($this->config->get('module_abandoned_carts_criteria') as $criteria) {
		...
		$sql .= " WHERE o.date_added >= DATE_SUB(NOW(), INTERVAL ".$this->config->get('module_abandoned_carts_limit')." DAY) && NOW() >= DATE_ADD(o.date_added, INTERVAL 1 HOUR)  && o.firstname !='' && o.order_status_id=0";
	...
	public function getTotalOrders($data = array()) {
		...
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` o WHERE o.date_added >= DATE_SUB(NOW(), INTERVAL ".$this->config->get('module_abandoned_carts_limit')." DAY) && NOW() >= DATE_ADD(o.date_added, INTERVAL 1 HOUR)  && o.firstname !='' && o.order_status_id=0 && o.abandoned = 0";
