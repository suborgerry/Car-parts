<?php
class ControllerExtensionQuickCheckoutGuest extends Controller {
  	public function index() {
		...
		if (isset($this->session->data['guest']['custom_field'])) {
			...
			$data['guest_custom_field'] = array_merge ($this->session->data['guest']['custom_field'], $this->session->data['payment_address']['custom_field']);
		} else {
			$data['guest_custom_field'] = array();
		}

		$data['shipping_required'] = $this->cart->hasShipping();
		$data['shipping_checkbox'] = $this->config->get('quickcheckout_field_shipping');
	...
	public function validate() {
		...
		if (!$json) {
			...
			if (!$pickup) {
				...
				if ($country_info && !empty($postcode['display'])) {
