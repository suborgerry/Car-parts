...
class ControllerExtensionQuickCheckoutShippingMethod extends Controller {
  	public function index() {
		...
		$data['shipping_title_display'] = $this->config->get('quickcheckout_shipping_title_display');
		...
		$data['locale'] = substr($this->session->data['language'],0,2);
	...
	public function validate() {
		...
		if (empty($shipping_address)&&isset($this->session->data['temp_address'])) {
			$this->session->data['shipping_address'] = $this->session->data['temp_address'];
			$shipping_address = $this->session->data['temp_address'];
		}
