...
if ($aProd['PRICES'] and !$aRes['FINDPRICE_BUTTON']) {
    ...
    if ($_COOKIE['PRICES_SORT']) {
        $prSort = json_decode($_COOKIE['PRICES_SORT']);
    }
    if (!$aRes['ACTIVE_TAB']) {
        $aRes['ACTIVE_TAB'] = 'LIST';
    }
...
?>
    ...
    ?>
        <div class="CmAvDelStWrap <? if (HIDE_PRODUCTS_COUNT) { ?>CmGrid1Fr<? } ?>">
            ...
            <? if (!HIDE_PRODUCTS_COUNT) { ?>
                <div class="CmDeliveryBlock <? if ($firstPr['DELIVERY_NUM'] == 0) {
                                                echo 'CmInStockDelivery';
                                            } else {
                                                echo 'CmTimeDelivery';
                                            } ?> CmTitShow" title="<?= Lng_x('Dtime_delivery', 0) ?>">
        ...
        <div class="CmPriceQuantBlWrap">
            <div class="CmDiscVatBlock">
                ...
                <? } ?>
                <div class="CmTablePrCost" style="color:#<? if ($firstPr['AVAILABLE_NUM'] == 0 && $aRes['ALLOW_NOTAVAIL'] == false) {
                                                                echo '808080';
                                                            } ?>;">
            ...
            <div class="cm_qtyCart">
                <? if ($firstPr['AVAILABLE_NUM'] > 0 || $aRes['ALLOW_NOTAVAIL']) {
                    ...
                    <? if ($aSets['ONECLICK_EMAIL_ORDER'] or $arConSets['ONECLICK_EMAIL_ORDER']) { ?>
                        <div class="CmMailOrder toCartButt CmColorBg" data-tab="AskPrice" data-tab="AskPrice" data-artnum="<?= $aProd['ArtNum'] ?>" data-brand="<?= $aProd['Brand'] ?>" data-moduledir="<?= CM_DIR ?>" data-lang="<?= LANG_x ?>" data-link="<?= $ProductURL ?>">
                            <span>
                                <svg class="CmCartImgPp" viewBox="0 2 24 24">
                    ...
                    <? } else { ?>
                        <div class="CmQuantBlToCartBl">
                            ...
                            <div class="CmAddToCart CmTablePrToCart CmColorBg" data-furl="<?= $aProd['Link'] ?>" data-priceid="<?= $firstPr['PriceID'] ?>">
                                <svg class="cm_HideCartImg" viewBox="-1 -3 24 24">
                                    <path d="M10 19.5c0 .829-.672 1.5-1.5 1.5s-1.5-.671-1.5-1.5c0-.828.672-1.5 1.5-1.5s1.5.672 1.5 1.5zm3.5-1.5c-.828 0-1.5.671-1.5 1.5s.672 1.5 1.5 1.5 1.5-.671 1.5-1.5c0-.828-.672-1.5-1.5-1.5zm-10.563-5l-2.937-7h16.812l-1.977 7h-11.898zm11.233-5h-11.162l1.259 3h9.056l.847-3zm5.635-5l-3.432 12h-12.597l.839 2h13.239l3.474-12h1.929l.743-2h-4.195z" />
                                </svg>
                            </div>
                        </div>
                ...
                <? } else { ?>
                    <div class="cm_NotAvailable_t">
                        <svg class="cm_NotAvImg" viewBox="0 -2 24 24">
    ...
    <? if (!$aSets['NOT_HIDE_PRICES'] or $aRes['ACTIVE_TAB'] == 'GRID') { ?>
        <div class="DelAvalStock" style="<? if ($aRes['ACTIVE_TAB'] == 'GRID') { ?>justify-content:center;<? } ?>">
            ...
            <? if (($aRes['SHOW_STOCK'] && $firstPr['SUPPLIER_STOCK'] != '') || ($aRes['SHOW_SUPPLIER'] && $firstPr['SUPPLIER_NAME'] != '')) { ?>
                <div class="CmSuplNameStockWrapBl">
                    <div class="svgStock">
                        <svg class="stockImg" viewBox="0 0 24 24" fill-rule="evenodd" clip-rule="evenodd">
                            <path d="M5 23h-2v-10l8.991-8.005c1.124.998 2.25 1.997 3.378 2.996l2.255 1.997c1.127.999 2.252 2.013 3.376 3.012v10h-2v-9.118l-7.009-6.215-6.991 6.22v9.113zm2-2h10v2h-10v-2zm0-3h10v2h-10v-2zm10-3v2h-10v-2h10zm-5-14l12 10.632-1.328 1.493-10.672-9.481-10.672 9.481-1.328-1.493 12-10.632z" />
                        </svg>
                    </div>
                    <? if ($aRes['SHOW_SUPPLIER'] && $firstPr['SUPPLIER_NAME'] != '') { ?>
        ...
        <? } ?>
        <div class="CmPriceProd" data-txta='<? if ($firstPr['AVAILABLE_NUM'] == 0) {
                                                echo Lng_x('Not_available');
                                            } else {
                                                if (HIDE_PRODUCTS_COUNT) {
                                                    echo Lng_x('Available');
                                                } else {
                                                    echo Lng_x('Availability');
                                                }
                                            } ?>' data-txtd='<? if ($firstPr['DELIVERY_NUM'] == 0) {
                                                                    echo Lng_x('In_stock');
                                                                } else {
                                                                    echo Lng_x('Dtime_delivery');
                                                                } ?>'>
            <div class="CmAvalBlPriceBl">
                <div class="CmAvalProdOptionWrap">
                    <div class="avalTd CmTitShow" title="<? if ($firstPr['AVAILABLE_NUM'] == 0) {
                                                                echo Lng_x('Not_available');
                                                            } else {
                                                                if (HIDE_PRODUCTS_COUNT) {
                                                                    echo Lng_x('Available');
                        ...
                        <div class="CmAvalImgTextPage <? if (HIDE_PRODUCTS_COUNT) { ?>CmPaddZ<? } ?> CmColorBr">
                            <? if (!HIDE_PRODUCTS_COUNT) { ?>
                                <div class="cm_svgAval CmColorFi"><svg class="CmAvalOnPage fillBg" viewBox="0 0 24 24">
                    ...
                    <? if ($firstPr['AVAILABLE_NUM'] > 0) { ?>
                        <div class="delivTd CmTitShow <? if (HIDE_PRODUCTS_COUNT) { ?>CmPaddUpDownZ<? } ?> <? if ($firstPr['DELIVERY_NUM'] == 0) {
                                                                                                                echo 'CmInStockDelivery" title="' . Lng_x('In_stock');
                                                                                                            } else {
                                                                                                                echo 'CmTimeDelivery" title="' . Lng_x('Dtime_delivery');
                                                                                                            } ?>" data-suplstock="<?= $firstPr['SUPPLIER_STOCK'] ?>">
                            <div class="svgDeliv">
                                <svg class="delivImg" viewBox="0 0 24 24" fill-rule="evenodd" clip-rule="evenodd">
                ...
                <? } else { ?>
                    <div class="CmQuantBlToCartBl">
                        <div class="blockQty">
                            ...
                            <input name="re_count" type="text" class="CmAddToCartQty quantProd cm_countRes CmColorTxi" value="<? if ($firstPr['OPTIONS_VIEW']['Minimal_qnt']) {
                                                                                                                                    echo $firstPr['OPTIONS_VIEW']['Minimal_qnt']['Text'];
                                                                                                                                } else { ?>1<? } ?>" data-maxaval="<?= $firstPr['AVAILABLE_NUM'] ?>" data-minimalqnt='<?= $firstPr['OPTIONS_VIEW']['Minimal_qnt']['Text'] ?>'>
                        ...
                        <? } else { ?>
                            <div class="CmAddToCart toCartButt CmColorBg a" data-furl="<?= $ProductURL ?>" data-priceid="<?= $firstPr['PriceID'] ?>">
                                <span>
                                    <svg class="CmCartImgPp" viewBox="0 2 24 24">
                                        <path d="M10 19.5c0 .829-.672 1.5-1.5 1.5s-1.5-.671-1.5-1.5c0-.828.672-1.5 1.5-1.5s1.5.672 1.5 1.5zm3.5-1.5c-.828 0-1.5.671-1.5 1.5s.672 1.5 1.5 1.5 1.5-.671 1.5-1.5c0-.828-.672-1.5-1.5-1.5zm-10.563-5l-2.937-7h16.812l-1.977 7h-11.898zm11.233-5h-11.162l1.259 3h9.056l.847-3zm5.635-5l-3.432 12h-12.597l.839 2h13.239l3.474-12h1.929l.743-2h-4.195z" />
                                    </svg>
            ...
            <? } else { ?>
                <div class="cmNoInStockForm" action="index.php?route=product/product/notify" method="post">
                    ...
                    <input id="cmNoInStockFormEmail" type="text" name="email" placeholder="Input your e-main" required>
                    <button id="cmNoInStockFormBtn" data-product-id="<?= $aProd['PKey'] ?>">Notify me</button>
                </div>
                <script>
                    (() => {
                        const btn = document.querySelector('#cmNoInStockFormBtn');
        ...
        <table class="CmTablePriceWrap">
                ...
                <tr class="CmTitleImg">
                        ...
                        <th>
                            <div id="cmdelnum" class="CmSvgDelivNotHide CmTitShow" title="<?= Lng_x('Dtime_delivery') ?>">
                                <? if (count($aProd['PRICES']) > 0) { ?>
                                    <div class="<? if ($prSort[0] == 'Delivery') { ?>CmColorFi<? } ?> CmSortBlock CmDescSort CmDelivery" data-sort="desc" data-val="Delivery" data-url="<?= $aRes['DETAIL_PAGE_URL'] ?>"><? if ($aPageSVG['CmSort']) {
                                                                                                                                                                                                                                echo $aPageSVG['CmSort'];
                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                                echo $aListSVG['CmSort'];
                                                                                                                                                                                                                            } ?></div>
                                <? } ?>
                                <svg class="<? if ($prSort[0] == 'Delivery' && count($aProd['PRICES']) > 0) { ?>CmColorFi<? } ?> cm_deliv" viewBox="0 0 24 24" fill-rule="evenodd" clip-rule="evenodd">
                                    <path d="M2 11.741c-1.221-1.009-2-2.535-2-4.241 0-3.036 2.464-5.5 5.5-5.5 1.706 0 3.232.779 4.241 2h4.259c.552 0 1 .448 1 1v2h4.667c1.117 0 1.6.576 1.936 1.107.594.94 1.536 2.432 2.109 3.378.188.312.288.67.288 1.035v4.48c0 1.156-.616 2-2 2h-1c0 1.656-1.344 3-3 3s-3-1.344-3-3h-4c0 1.656-1.344 3-3 3s-3-1.344-3-3h-2c-.552 0-1-.448-1-1v-6.259zm6 6.059c.662 0 1.2.538 1.2 1.2 0 .662-.538 1.2-1.2 1.2-.662 0-1.2-.538-1.2-1.2 0-.662.538-1.2 1.2-1.2zm10 0c.662 0 1.2.538 1.2 1.2 0 .662-.538 1.2-1.2 1.2-.662 0-1.2-.538-1.2-1.2 0-.662.538-1.2 1.2-1.2zm-7.207-11.8c.135.477.207.98.207 1.5 0 3.036-2.464 5.5-5.5 5.5-.52 0-1.023-.072-1.5-.207v4.207h1.765c.549-.614 1.347-1 2.235-1 .888 0 1.686.386 2.235 1h5.53c.549-.614 1.347-1 2.235-1 .888 0 1.686.386 2.235 1h1.765v-4.575l-1.711-2.929c-.179-.307-.508-.496-.863-.496h-4.426v6h-2v-9h-2.207zm5.207 4v3h5l-1.427-2.496c-.178-.312-.509-.504-.868-.504h-2.705zm-10.5-6c1.932 0 3.5 1.568 3.5 3.5s-1.568 3.5-3.5 3.5-3.5-1.568-3.5-3.5 1.568-3.5 3.5-3.5zm.5 3h2v1h-3v-3h1v2z" />
                                </svg>
                            </div>
                        </th>
                    ...
                    <? if ($noSuplStock) { ?>
                        <th>
                            <div class="CmSvgStockNotHide">
                                <svg class="cm_stock" viewBox="0 0 24 24" fill-rule="evenodd" clip-rule="evenodd">
                                    <path d="M5 23h-2v-10l8.991-8.005c1.124.998 2.25 1.997 3.378 2.996l2.255 1.997c1.127.999 2.252 2.013 3.376 3.012v10h-2v-9.118l-7.009-6.215-6.991 6.22v9.113zm2-2h10v2h-10v-2zm0-3h10v2h-10v-2zm10-3v2h-10v-2h10zm-5-14l12 10.632-1.328 1.493-10.672-9.481-10.672 9.481-1.328-1.493 12-10.632z" />
                                </svg>
                            </div>
                        </th>
                    ...
                    <th style="<? if ($aRes['ACTIVE_TAB'] == 'TABLE') { ?>display:none<? } ?>">
                        <div id="cmavnum" class="CmSvgAvalNotHide CmTitShow" title="<?= Lng_x('Availability') ?>">
                            <? if (count($aProd['PRICES']) > 0) { ?>
                                <div class="<? if ($prSort[0] == 'Available') { ?>CmColorFi<? } ?> CmSortBlock CmDescSort CmAvailable" data-sort="desc" data-val="Available" data-url="<?= $aRes['DETAIL_PAGE_URL'] ?>"><? if ($aPageSVG['CmSort']) {
                                                                                                                                                                                                                            echo $aPageSVG['CmSort'];
                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                            echo $aListSVG['CmSort'];
                                                                                                                                                                                                                        } ?></div>
                            <? } ?>
                            <svg class="<? if ($prSort[0] == 'Available' && count($aProd['PRICES']) > 0) { ?>CmColorFi<? } ?> CmAvalOnPage fillBg" viewBox="0 0 24 24">
                                <path d="M16.677 17.868l-.343.195v-1.717l.343-.195v1.717zm2.823-3.325l-.342.195v1.717l.342-.195v-1.717zm3.5-7.602v11.507l-9.75 5.552-12.25-6.978v-11.507l9.767-5.515 12.233 6.941zm-13.846-3.733l9.022 5.178 1.7-.917-9.113-5.17-1.609.909zm2.846 9.68l-9-5.218v8.19l9 5.126v-8.098zm3.021-2.809l-8.819-5.217-2.044 1.167 8.86 5.138 2.003-1.088zm5.979-.943l-2 1.078v2.786l-3 1.688v-2.856l-2 1.078v8.362l7-3.985v-8.151zm-4.907 7.348l-.349.199v1.713l.349-.195v-1.717zm1.405-.8l-.344.196v1.717l.344-.196v-1.717zm.574-.327l-.343.195v1.717l.343-.195v-1.717zm.584-.333l-.35.199v1.717l.35-.199v-1.717z" />
                            </svg>
                        </div>
                    </th>
                    ...
                    <th>
                        <div id="cmprnum" class="CmPriceTextTitle">
                            <? if (count($aProd['PRICES']) > 0) { ?>
                                <div class="<? if ($prSort[0] == 'Price') { ?>CmColorFi<? } ?> CmSortBlock CmDescSort CmPrice" data-sort="desc" data-val="Price" data-url="<?= $aRes['DETAIL_PAGE_URL'] ?>"><? if ($aPageSVG['CmSort']) {
                                                                                                                                                                                                                echo $aPageSVG['CmSort'];
                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                echo $aListSVG['CmSort'];
                                                                                                                                                                                                            } ?></div>
            ...
            <tbody class="CmTbodyPrice">
                <tr class="CmPriceValRow CmColorBgLh <? if ($firstPr['DELIVERY_NUM'] == 0) { ?>CmColorBgL<? } ?>" data-cmdelnum="<?= $firstPr['DELIVERY_NUM'] ?>" data-cmavnum="<?= $firstPr['AVAILABLE_NUM'] ?>" data-cmprnum="<?= strip_tags($firstPr['PRICE_VALUE']) ?>">
                    ...
                    <? } ?>
                    <td>
                        <div class="cm_AvalNotHide CmTitShow" title="<? if ($firstPr['AVAILABLE_NUM'] == 0) {
                                                                            echo Lng_x('Not_available');
                                                                        } else {
                                                                            echo Lng_x('Availability');
                                                                        } ?>">
                                    ...
                                    <div class="CmQuantBlToCartBl">
                                        ...
                                        <? } else { ?>
                                            <div class="CmQuantPriceBlock">
                                                ...
                                                <input name="re_count" type="text" class="CmAddToCartQty CmQuantInputProd cm_countRes" value="<? if ($firstPr['OPTIONS_VIEW']['Minimal_qnt']) {
                                                                                                                                                    echo $firstPr['OPTIONS_VIEW']['Minimal_qnt']['Text'];
                                                                                                                                                } else { ?>1<? } ?>" data-maxaval="<?= $firstPr['AVAILABLE_NUM'] ?>" data-minimalqnt='<?= $firstPr['OPTIONS_VIEW']['Minimal_qnt']['Text'] ?>'>
                                            ...
                                            <? } else { ?>
                                                <div class="CmAddToCart CmButtonToCart CmColorBg" data-furl="<?= $ProductURL ?>" data-priceid="<?= $firstPr['PriceID'] ?>">
                                                    <svg class="cm_HideCartImg" viewBox="0 0 24 24">
                                                        <path d="M10 19.5c0 .829-.672 1.5-1.5 1.5s-1.5-.671-1.5-1.5c0-.828.672-1.5 1.5-1.5s1.5.672 1.5 1.5zm3.5-1.5c-.828 0-1.5.671-1.5 1.5s.672 1.5 1.5 1.5 1.5-.671 1.5-1.5c0-.828-.672-1.5-1.5-1.5zm-10.563-5l-2.937-7h16.812l-1.977 7h-11.898zm11.233-5h-11.162l1.259 3h9.056l.847-3zm5.635-5l-3.432 12h-12.597l.839 2h13.239l3.474-12h1.929l.743-2h-4.195z" />
                                                    </svg>
                                                </div>
                                            <? } ?>
                    ...
                    foreach ($aProd['PRICES'] as $k => $aPrice) {
                        ...
                        if ($tr <= 3) { ?>
                            <tr class="CmTablePriceValueRow <? if ($aPrice['DELIVERY_NUM'] == 0) { ?>CmColorBgL<? } ?>" data-cmdelnum="<?= $aPrice['DELIVERY_NUM'] ?>" data-cmavnum="<?= $aPrice['AVAILABLE_NUM'] ?>" data-cmprnum="<?= strip_tags($aPrice['PRICE_VALUE']) ?>">
                                ...
                                <? } ?>
                                <td>
                                    <div class="cm_AvalNotHide CmTitShow" title="<? if ($aPrice['AVAILABLE_NUM'] == 0) {
                                                                                        echo Lng_x('Not_available');
                                                                                    } else {
                                                                                        echo Lng_x('Availability');
                                                                                    } ?>">
                                ...
                                <td>
                                    <div class="CmPriceChangeQuant">
                                        ...
                                        <div class="CmPriceQuantWrap">
                                            <div class="CmPriceNum" style="color:#<? if ($aPrice['AVAILABLE_NUM'] == 0 && $aRes['ALLOW_NOTAVAIL'] == false) {
                                                                                        echo '808080';
                                                                                    } ?>;">
                                                <span>
                                                    <nobr><?= $aPrice['PRICE_VALUE']; ?></nobr>
                                                </span>
                                            </div>
                                            <? if ($aPrice['AVAILABLE_NUM'] > 0 || $aRes['ALLOW_NOTAVAIL']) { ?>
                                                <div class="CmQuantBlToCartBl">
                                                    ...
                                                    <? } else { ?>
                                                        <div class="CmQuantPriceBlock">
                                                            <div class="CmQuantMinusBut CmColorTxh cm_countButM">-</div>
                                                            <input name="re_count" type="text" class="CmAddToCartQty CmQuantInputProd cm_countRes" value="<? if ($aPrice['OPTIONS_VIEW']['Minimal_qnt']) {
                                                                                                                                                                echo $aPrice['OPTIONS_VIEW']['Minimal_qnt']['Text'];
                                                                                                                                                            } else { ?>1<? } ?>" data-maxaval="<?= $aPrice['AVAILABLE_NUM'] ?>" data-minimalqnt='<?= $aPrice['OPTIONS_VIEW']['Minimal_qnt']['Text'] ?>'>
                                                            <div class="CmQuantPlusBut CmColorTxh cm_countButP">+</div>
                                                        </div>
                                                        <? if ($aRes['FINDPRICE_DISPLAY_PRICES']) { ?>
                                                            <a href="<?= $ProductURL ?>" class="CmFPrNotHideLink CmColorBg" <?= $aRes['FindPrice_isBlank'] ?>>
        ...
        <div class="CmAvalAskPrBlock">
            <? if ($aRes['FINDPRICE_BUTTON']) { ?>
                <a href="<?= $aProd['FindPriceLink'] ?>" class="CmPriceAskBut CmColorBg CmColorBr" <?= $aRes['FindPrice_isBlank'] ?>>
                    <svg class="CmAskImg CmColorFi" viewBox="0 0 24 24">
            ...
            <? } else { ?>
                <? if ($aRes['ASK_PRICE']) { ?>
                    <div class="ListAskPrice_t CmAskPrice" data-artnum="<?= $aProd['ArtNum'] ?>" data-brand="<?= $aProd['Brand'] ?>" data-moduledir="<?= CM_DIR ?>" data-lang="<?= LANG_x ?>">
                        <svg class="cm_askImg_t" viewBox="0 0 24 24">
                            <path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm1.25 17c0 .69-.559 1.25-1.25 1.25-.689 0-1.25-.56-1.25-1.25s.561-1.25 1.25-1.25c.691 0 1.25.56 1.25 1.25zm1.393-9.998c-.608-.616-1.515-.955-2.551-.955-2.18 0-3.59 1.55-3.59 3.95h2.011c0-1.486.829-2.013 1.538-2.013.634 0 1.307.421 1.364 1.226.062.847-.39 1.277-.962 1.821-1.412 1.343-1.438 1.993-1.432 3.468h2.005c-.013-.664.03-1.203.935-2.178.677-.73 1.519-1.638 1.536-3.022.011-.924-.284-1.719-.854-2.297z" />
                        </svg>
                ...
                if ($aRes['ALLOW_ORDER']) { ?>
                    <div class="CmAddToCart ListNotAvailable_t" data-furl="<?= $aProd['Link'] ?>" data-priceid="order">
                        <svg class="cm_cartImg_t" viewBox="0 0 24 24">
                            <path d="M10 19.5c0 .829-.672 1.5-1.5 1.5s-1.5-.671-1.5-1.5c0-.828.672-1.5 1.5-1.5s1.5.672 1.5 1.5zm3.5-1.5c-.828 0-1.5.671-1.5 1.5s.672 1.5 1.5 1.5 1.5-.671 1.5-1.5c0-.828-.672-1.5-1.5-1.5zm-10.563-5l-2.937-7h16.812l-1.977 7h-11.898zm11.233-5h-11.162l1.259 3h9.056l.847-3zm5.635-5l-3.432 12h-12.597l.839 2h13.239l3.474-12h1.929l.743-2h-4.195z" />
                        </svg>
    ...
    <? } else { ?>
        <div class="AvalAsk" data-artnum="<?= $aProd['ArtNum'] ?>" data-brand="<?= $aProd['Brand'] ?>">
            ...
            <? } else { ?>
                <div class="cmNoInStock CmColorBr m">
                ...
                <div class="CmAksNotAvWrapBl">
                    <? if ($aRes['ASK_PRICE']) { ?>
                        <div class="ListAskPrice" data-tab="AskPrice" data-tab="AskPrice" data-artnum="<?= $aProd['ArtNum'] ?>" data-brand="<?= $aProd['Brand'] ?>" data-moduledir="<?= CM_DIR ?>" data-lang="<?= LANG_x ?>" data-link="<?= $ProductURL ?>">
                            <svg class="CmAskImg" viewBox="0 0 24 24">
                                <path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm1.25 17c0 .69-.559 1.25-1.25 1.25-.689 0-1.25-.56-1.25-1.25s.561-1.25 1.25-1.25c.691 0 1.25.56 1.25 1.25zm1.393-9.998c-.608-.616-1.515-.955-2.551-.955-2.18 0-3.59 1.55-3.59 3.95h2.011c0-1.486.829-2.013 1.538-2.013.634 0 1.307.421 1.364 1.226.062.847-.39 1.277-.962 1.821-1.412 1.343-1.438 1.993-1.432 3.468h2.005c-.013-.664.03-1.203.935-2.178.677-.73 1.519-1.638 1.536-3.022.011-.924-.284-1.719-.854-2.297z" />
                            </svg>
                            <span><?= Lng_x('Ask_price') ?></span>
                        </div>
                    ...
                    if ($aRes['ALLOW_ORDER']) { ?>
                        <div class="CmAddToCart ListNotAvailable CmColorBg" data-furl="<?= $ProductURL ?>" data-priceid="order">
                            <svg class="cm_cartImg" viewBox="0 0 24 24">
                                <path d="M10 19.5c0 .829-.672 1.5-1.5 1.5s-1.5-.671-1.5-1.5c0-.828.672-1.5 1.5-1.5s1.5.672 1.5 1.5zm3.5-1.5c-.828 0-1.5.671-1.5 1.5s.672 1.5 1.5 1.5 1.5-.671 1.5-1.5c0-.828-.672-1.5-1.5-1.5zm-10.563-5l-2.937-7h16.812l-1.977 7h-11.898zm11.233-5h-11.162l1.259 3h9.056l.847-3zm5.635-5l-3.432 12h-12.597l.839 2h13.239l3.474-12h1.929l.743-2h-4.195z" />
                            </svg>
                            <span><?= Lng_x('Order') ?></span>
                        </div>
        ...
        <div class="CmMorePrices CmColorTx" <? if ($aRes['ACTIVE_TAB'] == 'TABLE') { ?>style="margin-top:0px;" <? } ?>>&#9660; <?= Lng_x('Show_more_prices') ?> (<?= count($aProd['PRICES']) ?>)
            <div class="morePricestab" style="position:absolute;">
                        ...
                        <tr class="CmTitleImg">
                            <? if (!HIDE_PRODUCTS_COUNT) { ?>
                                <th>
                                    <div id="cmdelnum" class="CmSvgDelivNotHide">
                                        ...
                                        <? } ?>
                                        <svg class="<? if ($prSort[0] == 'Delivery') { ?>CmColorFi<? } ?> cm_deliv" viewBox="0 0 24 24" fill-rule="evenodd" clip-rule="evenodd">
                                            <path d="M2 11.741c-1.221-1.009-2-2.535-2-4.241 0-3.036 2.464-5.5 5.5-5.5 1.706 0 3.232.779 4.241 2h4.259c.552 0 1 .448 1 1v2h4.667c1.117 0 1.6.576 1.936 1.107.594.94 1.536 2.432 2.109 3.378.188.312.288.67.288 1.035v4.48c0 1.156-.616 2-2 2h-1c0 1.656-1.344 3-3 3s-3-1.344-3-3h-4c0 1.656-1.344 3-3 3s-3-1.344-3-3h-2c-.552 0-1-.448-1-1v-6.259zm6 6.059c.662 0 1.2.538 1.2 1.2 0 .662-.538 1.2-1.2 1.2-.662 0-1.2-.538-1.2-1.2 0-.662.538-1.2 1.2-1.2zm10 0c.662 0 1.2.538 1.2 1.2 0 .662-.538 1.2-1.2 1.2-.662 0-1.2-.538-1.2-1.2 0-.662.538-1.2 1.2-1.2zm-7.207-11.8c.135.477.207.98.207 1.5 0 3.036-2.464 5.5-5.5 5.5-.52 0-1.023-.072-1.5-.207v4.207h1.765c.549-.614 1.347-1 2.235-1 .888 0 1.686.386 2.235 1h5.53c.549-.614 1.347-1 2.235-1 .888 0 1.686.386 2.235 1h1.765v-4.575l-1.711-2.929c-.179-.307-.508-.496-.863-.496h-4.426v6h-2v-9h-2.207zm5.207 4v3h5l-1.427-2.496c-.178-.312-.509-.504-.868-.504h-2.705zm-10.5-6c1.932 0 3.5 1.568 3.5 3.5s-1.568 3.5-3.5 3.5-3.5-1.568-3.5-3.5 1.568-3.5 3.5-3.5zm.5 3h2v1h-3v-3h1v2z" />
                                        </svg>
                                    </div>
                                </th>
                            ...
                            <th style="">
                                <div id="cmavnum" class="CmSvgAvalNotHide">
                                    ...
                                    <? } ?>
                                    <svg class="<? if ($prSort[0] == 'Available') { ?>CmColorFi<? } ?> CmAvalOnPage fillBg" viewBox="0 0 24 24">
                                        <path d="M16.677 17.868l-.343.195v-1.717l.343-.195v1.717zm2.823-3.325l-.342.195v1.717l.342-.195v-1.717zm3.5-7.602v11.507l-9.75 5.552-12.25-6.978v-11.507l9.767-5.515 12.233 6.941zm-13.846-3.733l9.022 5.178 1.7-.917-9.113-5.17-1.609.909zm2.846 9.68l-9-5.218v8.19l9 5.126v-8.098zm3.021-2.809l-8.819-5.217-2.044 1.167 8.86 5.138 2.003-1.088zm5.979-.943l-2 1.078v2.786l-3 1.688v-2.856l-2 1.078v8.362l7-3.985v-8.151zm-4.907 7.348l-.349.199v1.713l.349-.195v-1.717zm1.405-.8l-.344.196v1.717l.344-.196v-1.717zm.574-.327l-.343.195v1.717l.343-.195v-1.717zm.584-.333l-.35.199v1.717l.35-.199v-1.717z" />
                                    </svg>
                                </div>
                            </th>
                ...
                <div class="CmMorePriceBlClose">
                    <svg class="CmCloseMorePr CmColorFi" viewBox="0 0 24 24">
...
if (WS_ACTIVE) { ?>
    <div class="CmWsLoadBar">
        <div class="CmWsLBCh"></div>
        <div class="CmWsLBCh"></div>
    </div>
<? }
AjaxCut_x('ProductPrices' . $PodPriceNum);
